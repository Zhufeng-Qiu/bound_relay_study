...
| container overlay, durable | 3.47 GB/s | compression pays — ~~implied~~ **measured 2.07×** |
| MooseFS, durable | 0.43 GB/s | ~~compression pays 3×~~ — **measured 1.59×** |
| MooseFS as the pipeline used it, unsynced round trip | — | compression loses, 1.17× — measured |

**The two projected rows were arithmetic over a bandwidth sweep and both were
optimistic.** `fsync_offload_findings.md` measured the offload write itself:
compression pays on every configuration of both filesystems, at **1.21×–2.10×** and
never at the 3.02× the byte reduction implies, because a 77.7 MB write is given
0.55× the bandwidth a 234.9 MB write gets on that mount. The projection assumed a
bandwidth independent of the payload, which is the same shape of error as assuming
a codec's throughput is independent of tensor size — the mistake Phase C found.

The last row is not comparable to the two above it: it is a round trip through a
two-thread ring in another data centre, not an offload write.
