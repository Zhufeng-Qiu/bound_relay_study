...
| 10 | 2026-09-03 | `09xqdxdm9y28dd` | 2×RTX A6000 | **Gate 2 re-run** — the 30% overlap figure was measured with uninitialised buffers and is flagged unquotable. A40 is out of stock at 2 GPUs, so both the buggy and the fixed variant run here back to back and the hardware change cancels. | `gate2_rerun.json`: encode/decode/1-thread/2-thread on the same host under both buffer regimes, with a reconstruction check the original never did | $1.00 | | |
