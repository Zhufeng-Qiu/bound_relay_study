...
PAIRS = [(0.005, 0.03), (0.005, 0.10), (0.01, 0.03), (0.01, 0.10),
         (0.01, 0.01), (0.03, 0.03)]        # last two are symmetric references
...
for di,(title, body) in enumerate(docs):
    ...
    for pair in PAIRS:
        ...
        rows.append({"doc":f"d{di:02d}","strategy":f"cK{pair[0]:g}_cV{pair[1]:g}",
...
for pair in PAIRS:
    for _ in (0,):
        strat=f"cK{pair[0]:g}_cV{pair[1]:g}"; c=None
        ...
        print(f"{pair[0]:>6g} {pair[1]:>6g} {by/1e6:10.1f} {by/base_bytes:7.2f}x {np.exp(nl):8.3f} "
              f"{nl-base_nll:+9.4f}  [{lo:+.4f},{hi:+.4f}]{'  *' if not (lo<0<hi) else ''}")
json.dump(out, open("/workspace/quality_asym.json","w"), indent=2)
